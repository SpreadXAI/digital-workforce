#!/usr/bin/env python3
"""Deploy Digital Workforce to Singapore ECS via Aliyun Cloud Assistant."""
...
INSTANCE = os.environ.get("DW_INSTANCE", "i-t4n571mpi11mkib9ubxj")
TACTILE_INSTANCE = os.environ.get("TACTILE_DB_SOURCE_INSTANCE", "i-t4n30tqeuanmlmhswi5h")
API_PORT = int(os.environ.get("DW_API_PORT", "9092"))
...
async def main() -> None:
    async with async_session_factory() as db:
        for email in ("digital-workforce-bridge@spreadx.ai", "spider-radar-bridge@spreadx.ai"):
...
# Retire spider-radar
systemctl stop spider-radar-api 2>/dev/null || true
systemctl disable spider-radar-api 2>/dev/null || true
rm -f /etc/nginx/sites-enabled/spider-radar
...
REMOTE_DIR=/opt/digital-workforce
WEB_DIR=/var/www/digital-workforce
SERVICE=digital-workforce-api
...
TACTILE_SKILL_CREATOR_AGENT_ID={esc(tactile.get("SKILL_CREATOR_AGENT_ID", "0"))}
DW_PUBLIC_API_BASE={esc(tactile.get("DW_API_BASE", "http://43.98.185.179/api"))}
EOF
...
[Unit]
Description=Digital Workforce API
After=network.target

[Service]
Type=simple
WorkingDirectory=/opt/digital-workforce/backend
EnvironmentFile=/opt/digital-workforce/backend/.env
Environment=PYTHONPATH=/opt/digital-workforce/backend
ExecStart=/opt/digital-workforce/.venv/bin/uvicorn app.main:app --host 127.0.0.1 --port {API_PORT}
Restart=always
RestartSec=3
...
cat > /etc/nginx/sites-available/digital-workforce << NGINX
server {{
    listen 80 default_server;
    listen [::]:80 default_server;
    server_name _;
    root /var/www/digital-workforce;
    index index.html;
...
ln -sf /etc/nginx/sites-available/digital-workforce /etc/nginx/sites-enabled/digital-workforce
nginx -t
...
def main() -> None:
    jwt_secret = os.environ.get("JWT_SECRET", "digital-workforce-prod-jwt-secret-4918")
    gh_token = os.environ.get("github_access_token") or os.environ.get("GH_TOKEN", "")

    print("==> Push latest to GitHub")
    subprocess.run(["git", "add", "-A"], cwd=ROOT, check=False)
    subprocess.run(
        ["git", "commit", "-m", "chore: deploy digital workforce to Singapore", "--allow-empty"],
        cwd=ROOT,
        check=False,
    )
    subprocess.run(["git", "push", "origin", "main"], cwd=ROOT, check=True)

    clone_url = "https://github.com/SpreadXAI/digital-workforce.git"
    if gh_token:
        clone_url = f"https://{gh_token}@github.com/SpreadXAI/digital-workforce.git"
    ...
    tactile = {
        ...
        "SKILL_CREATOR_AGENT_ID": os.environ.get("TACTILE_SKILL_CREATOR_AGENT_ID", "0"),
        "DW_API_BASE": os.environ.get("DW_PUBLIC_API_BASE", "http://43.98.185.179/api"),
    }
    if not tactile["API_KEY"]:
        print("==> Fetch bridge API key from Tactile gateway")
        tactile["API_KEY"] = fetch_tactile_api_key()

    print(f"==> Bootstrap digital-workforce on {INSTANCE} ({REGION})")
    ...
    print("    Admin: admin@spreadx.ai / Dw@Admin2026")
    print("    QA:    qa@spreadx.ai / Dw@Test2026")
